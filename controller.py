    ...
    def run(self):
        ...
        while True:
            ...
            if user_choice == "1":
                ...
                id_contact = self.view.display_contacts(
                    self.model.contacts, 20, "update"
                )
                if id_contact:
                    contact_update = asdict(self.model.contacts[id_contact - 1])
                    result_update_contact = self.view.update_contact_info(
                        contact_update
                    )
                    self.model.update_contact(
                        id_contact - 1,
                        Contact(
                            result_update_contact["id"],
                ...
                del_contact_id = self.view.display_contacts(
                    self.model.contacts, 20, "delete"
                )
                if del_contact_id:
